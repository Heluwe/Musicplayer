...
namespace XiMusic
{
    public class HtmlHelperEncode
    {
        public static string HexString(string s, Encoding encoding)
        {
        ...
        public static string CreateEncode(string singer, string title, int lrcId)
        {
            string hexStr = HexString(singer + title, Encoding.UTF8);
            int length = hexStr.Length / 2;
            int[] song = new int[length];
            for (int i = 0; i < length; i++)
            {
                song[i] = int.Parse(hexStr.Substring(i * 2, 2), System.Globalization.NumberStyles.HexNumber);
            }
            int t1 = 0, t2 = 0, t3 = 0;
